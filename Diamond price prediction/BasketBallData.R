...
#Field Goal Attempts
FieldGoalsAttempts <-rbind(KobeBryant_FGA,JoeJohnson_FGA,LeBronJames_FGA,CarmeloAnthony_FGA,DwightHoward_FGA,ChrisBosh_FGA,ChrisPaul_FGA,KevinDurant_FGA,DerrickRose_FGA,DwayneWade_FGA)
rownames(MinutesPlayed) <- Players
colnames(MinutesPlayed) <-Seasons
...
#FreeThrowsAttempts.data <- data.frame
...
#
#Data preperation for visualizations

FreeThrowsAttempts.Game <- as.data.frame(round(FreeThrowsAttempts/Games,2))
colnames(FreeThrowsAttempts.Game) <- Seasons
#Add a column Player Name
FreeThrowsAttempts.Game$Players <-Players
#FreeThrowsAttempts.Game$ <- NULL



FreeThrowAccuracy <- data.frame(round(FreeThrows/FreeThrowsAttempts,2)*100)
colnames(FreeThrowAccuracy) <- Seasons
FreeThrowAccuracy$Players <- Players



PlayerPlayingStyle <-data.frame(round((Points-FreeThrows)/FieldGoals))
colnames(PlayerPlayingStyle) <- Seasons
PlayerPlayingStyle$Players <- Players

#Visualize using ggplot

#plot for FreeThrowAttempts ,transfer  to  a data frame before using ggplot,,need to try to chnage it to  data frame 
ggplot(data=FreeThrowsAttempts.Game,aes(x=Players,y=Seasons,color=Players))+geom_point()+ggtitle("FreeThrow")

ggplot(data=FreeThrowAccuracy,aes(x=Players,y=Seasons,color=Players))+geom_point()

ggplot(data=PlayerPlayingStyle,aes(x=Players,y=Seasons,color=Players))+geom_point()
