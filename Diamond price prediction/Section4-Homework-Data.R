...
#Matrix
FreeThrowsATA <- rbind(KobeBryant_FTA,JoeJohnson_FTA,LeBronJames_FTA,CarmeloAnthony_FTA,DwightHoward_FTA,
                    ChrisBosh_FTA,ChrisPaul_FTA, KevinDurant_FTA, DerrickRose_FTA, DwayneWade_FTA )
rownames(FreeThrowsATA) <- Players
colnames(FreeThrowsATA) <- Seasons


#Free Throw attempts per Game
FreeThrowsATA.Game <- round(FreeThrowsATA/Games,1)

#Free throw accuracy
FreeThrowsAccruacy <- round(FreeThrows/FreeThrowsATA,1)*100

#Player playing style 

points.after.free.throws <-round((Points - FreeThrows )/FieldGoals,1)
